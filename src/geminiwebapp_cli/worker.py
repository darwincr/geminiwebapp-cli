...
from geminiwebapp_cli.conf import WORKER_IDLE_TIMEOUT_S, geminiwebapp_cli_home
from geminiwebapp_cli.session import GeminiSession, session_lock
from geminiwebapp_cli.session import _locks_dir

CONNECT_TIMEOUT_S = 60


def _worker_dir() -> Path:
    path = geminiwebapp_cli_home() / "workers"
...
def _log_path(name: str) -> Path:
    path = geminiwebapp_cli_home() / "logs"
    path.mkdir(parents=True, exist_ok=True)
...
def _send_request(path: Path, payload: dict, *, timeout: float | None = None) -> dict:
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
        ...
        while True:
            ...
            chunks.append(chunk)
    if not chunks:
        raise RuntimeError("geminiwebapp-cli worker closed the connection without a response")
...
def _startup_error(name: str, reason: str) -> RuntimeError:
    message = f"geminiwebapp-cli worker for session {name!r} {reason}"
...
def _start_worker(name: str) -> subprocess.Popen:
    ...
    env = os.environ.copy()
    env["GEMINIWEBAPP_CLI_WORKER"] = "1"
    return subprocess.Popen(
        [sys.executable, "-m", "geminiwebapp_cli.worker", name],
...
def _execute_request(session: GeminiSession, argv: list[str]) -> dict:
    import contextlib
    import io

    from geminiwebapp_cli.cli import _execute_verb, _parse_args
...
def serve(name: str) -> int:
    ...
    with session_lock(name), GeminiSession(name) as session:
        try:
            path.unlink()
        except FileNotFoundError:
            pass
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print("usage: python -m geminiwebapp_cli.worker <session>", file=sys.stderr)
        return 2
    return serve(args[0])
